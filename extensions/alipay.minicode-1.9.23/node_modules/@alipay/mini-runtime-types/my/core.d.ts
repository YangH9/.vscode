export interface MyBridge {
  ...
  readonly env: {
     ...
     * @sdk 2.7.24
     */
    clientEnv?: 'prod' | 'stable' | 'test' | 'pre' | 'unknown';
  };
  /**
   * 判断当前是否为 IDE 环境
   */
   ...
   * 创建离屏 canvas 对象
   */
  _createOffscreenCanvas(width?: number, height?: number, type?: '2d' | 'webgl'): OffScreenCanvas;
  /**
   * 触发目标元素相交事件
   */
  _fireIntersectionObserver(): void;
  /**
   * 小程序内唤起开卡页面。
  ...
  addEventCal(r: {
    ...
    endDate: string;
    /**
     * 循环发生次数，结合frequency一起使用
     * @default 0
     */
    recurrenceTimes?: number;
    /**
     * 循环频率，结合recurrenceTimes一起使用
     */
  ...
  addEventCalendar(r: {
    ...
    title: string;
    /**
     * 优先将 startDate 按照日期转换到 UNIX 时间戳
     */
    startDate?: string;
    /**
     * UNIX 时间戳
  ...
  addPeripheralService(r: {
     ...
     * 特征值列表。
     */
    characteristics: IAddPeripheralServiceCharacteristics[];
    /**
     * 接口调用成功的回调函数
     */
    success?(data: { success: true }): void;
  ...
  addTabBarItem(r: {
    ...
    activeIcon?: string;
    /**
     * tabItem 对应的页面路径，需要配置在小程序配置文件中
     */
    pagePath: string;
    /**
     * item 插入位置，原位置的页面将后移一个位置，从 `0` 开始
     */
    index: number;
    /**
     * 接口调用成功的回调函数
  ...
  /**
   * 追加渠道透传信息
   * @deprecated 实际不支持 appendChinfoChain
   */
  appendChinfoChain(r: {
    chinfo: string;
    scm: string;
  ...
  authorize(r: {
     ...
     * 需要获取权限类型
     */
    scopes: `${EGetAuthorizeScopeNicks}` | Array<`${EGetAuthorizeScopeNicks}`>;
    /**
     * 接口调用成功的回调函数
     */
    success?(data: {
       ...
       * 失败的授权类型
       */
      authErrorScopes: IMyApAuthorizeAuthErrorScopes;
      /**
       * 成功的授权 scope
       */
      authSuccessScopes: Array<`${EGetAuthorizeScopeNicks}`>;
    }): void;
    /**
     * 接口调用失败的回调函数
     */
   ...
   * 获取本机支持的 IFAA 生物认证方式
   */
  checkIsSupportIfaaAuthentication(r?: {
    /**
     * 接口调用成功的回调函数
     */
    success?(
   ...
   * 从设备存储中选取文件
   */
  chooseFileFromDisk(r?: {
    /**
     * 对应的appid，选择的文件会拷贝到这个appid对应的临时目录中，小程序默认从上下文获取
     */
    appId?: string;
    /**
     * 接口调用成功的回调函数
  ...
  /**
   * 创建并返回 <ai-camera> 组件上下文
   */
  createAICameraContext(id: string): AICameraContext;
  /**
   * 创建动画实例
   ...
   * 创建并返回 <ar> 组件上下文
   */
  createARContext(id: string): ARContext;
  /**
   * 创建并返回 <audio> 组件上下文
   */
  createAudioContext(id: string): AudioContext;
  /**
   * 创建并返回 <camera> 组件上下文
   * @see https://opendocs.alipay.com/mini/03qfj7
   */
   ...
   * 创建并返回保险屏幕操作录制会话
   */
  createInsuranceXReplaySession(param: {
    /**
     * 保险单号
     */
    prodNo: string;
  }): InsuranceXReplaySession;
  /**
   * 创建并返回 IntersectionObserver 对象
   * @see https://opendocs.alipay.com/mini/api/intersectionobserver
   */
  createIntersectionObserver(option?: IMyCreateIntersectionObserverOption): IntersectionObserver;
  /**
   * 创建并返回 <live-player> 组件上下文
   */
  createLivePlayerContext(id: string): LivePlayerContext;
  /**
   * 创建并返回 <live-pusher> 组件上下文
   */
  createLivePusherContext(id: string): LivePusherContext;
  /**
   * 创建并返回 <lottie> 组件上下文
   * @see https://opendocs.alipay.com/mini/api/createlottiecontext
   */
  createLottieContext(lottieId: string): LottieContext;
  /**
   * 创建并返回 <map> 组件上下文
   * @see https://opendocs.alipay.com/mini/api/ui-map
   */
  createMapContext(id: string): MapContext;
  /**
   * 创建离屏 canvas 对象
   */
  createOffscreenCanvas(width?: number, height?: number, type?: '2d' | 'webgl'): OffScreenCanvas;
  /**
   * 创建并返回安全风控上下文
   */
  createRDSContext(option?: IMyCreateRDSContextOption): RDSContext;
  /**
   * 创建并返回 <rtc-room> 组件上下文
   */
  createRtcRoomContext(id: string): RtcRoomContext;
  /**
   * 创建视图信息查询实例
   ...
   */
  createSelectorQuery(option?: IMyCreateSelectorQueryOption): SelectorQuery;
  /**
   * 创建 TCPSocket 实例
   */
  createTCPSocket(param?: {
    /**
     * 绑定到当前 WiFi
   ...
   * 创建 Xnn 引擎实例
   */
  createXnnManager(id: string): XnnManager;
  /**
   * 发起 Alipay+ 国际支付
   */
  crossPay(r?: {
  ...
  /**
   * 退出当前小程序
   * @deprecated 请使用 [my.exitMiniProgram]()
   */
  exit(r?: {
    /**
     * 接口调用成功的回调函数
  ...
  getARSessionManager(): ARSessionManager;
  /**
   * 获取授权码 (AuthCode)
   */
  getAuthCode(r?: {
  ...
  /**
   * 获取后台音乐播放状态
   * @deprecated 请使用 [my.getBackgroundAudioManager]() 代替
   */
   ...
   * 获取支付宝会员基础信息
   * @description
   * - 支付宝会员的基础信息包括（头像图片地址、昵称、性别、国家码、省份、所在市区）
   * - 如需获取支付宝会员标识（user_id），请调用 [my.getAuthCode]()
   */
  getCustomOpenUserInfo(r: {
    /**
     * scope
     */
    scope: 'share_userinfo' | 'share_phonenumber' | 'share_idnumber' | 'share_idphone' | 'share_membergrage' | 'share_userinfo_ext';
    /**
     * 接口调用成功的回调函数
     */
    success?(data: {
  ...
  getDeviceID(r?: {
    ...
    success?(data: {
       ...
       * Android 10以下版本返回 IMEI，经过无线保镖加密;
       * Android 10以上版本返回 OAID，经过无线保镖加密;
       */
      data: string;
      /**
       * 设备唯一标识类型
       * @description
       * iOS 返回 1 (代表：IDFA);
       * Android 10以下版本返回 2（ 代表：IMEI）；
       * Android 10以上版本返回 3 （代表OAID）；
       * @native 10.2.8
       */
      type: number;
      umidToken: string;
  ...
  /**
   * 获取设备信息
   * @description 获得设备加密标识后需要在服务端进行解密
   */
  getDeviceInfo(r?: {
    /**
     * 接口调用成功的回调函数
    ...
    success?(data: {
      /**
       * @description 获得设备加密标识，通过无线保镖加密，**需要在服务端进行解密**。
       */
      data: string;
    }): void;
    /**
   ...
   */
  getExtConfigSync(): Record<string, unknown>;
  /**
   * 获取文件信息
   ...
   */
  getSystemInfoSync(): TypeSystemInfo;
  /**
   * 同步获取设备设置
   */
  getSystemSetting(): {
  ...
  /**
   * 隐藏所有页面的收藏功能
   * @deprecated 该接口下线，详见 [21.11.15 隐藏收藏 API 下线](https://yuque.antfin.com/docs/share/a9d2bcbd-c033-4557-8bc9-c6e2336529e4?#)，如需 "隐藏收藏" 请联系 @芷兰
   * @see https://opendocs.alipay.com/mini/api/glp0w0
   */
  hideAllFavoriteMenu(r?: {
    /**
     * 接口调用成功的回调函数
     */
    success?(data: {}): void;
   ...
   * 隐藏基础库调试信息
   */
  hideDebugVersion(hide?: boolean): void;
  /**
   * 隐藏当前页面的收藏按钮
   * @deprecated 该接口已下线，详见 [21.11.15 隐藏收藏 API 下线](https://yuque.antfin.com/docs/share/a9d2bcbd-c033-4557-8bc9-c6e2336529e4?#)，如需 "隐藏收藏" 请联系 @芷兰
   * @see https://opendocs.alipay.com/mini/api/avtg7h
   */
  hideFavoriteMenu(r?: {
    /**
     * 接口调用成功的回调函数
  ...
  httpRequest(r: {
    ...
    url: string;
    /**
     * 设置请求的 HTTP 头对象，该对象里面的 `key` 和 `value` 必须是 `string` 类型。
  ...
  iotCheckService(r?: {
    /**
     * 该服务的id，由协议服务商定义，协议服务商appId范围内唯一即可，用于单个协议服务商唯一标识一个服务(选填，如果不指定则创建默认服务实例)
     */
    serviceId?: string;
    /**
     * 接口调用成功的回调函数
  ...
  iotCreateService(r: {
    ...
    serviceName: string;
    /**
     * 服务备注，如果不填，则默认使用协议服务商小程序的名称
     */
    remark?: string;
    /**
     * url跳转参数对象，服务创建后用户可点击服务进入协议服务商的小程序(通过形如alipays://platformapi/startapp?appId=xxxxx&key1=value1&key2=value2…)，该字段用于给小程序代入额外业务参数
     */
    urlParams?: string;
    /**
     * 接口调用成功的回调函数
  ...
  /**
   * 查询当前小程序是否已经被收藏
   * @see https://opendocs.alipay.com/mini/api/hlxgya
   */
  isCollected(r?: {
    /**
     * 接口调用成功的回调函数
    ...
    success?(data: {
      /**
       * 当前小程序是否收藏，`true` 为已收藏，`false` 为未收藏。
       */
      isCollected: boolean;
    }): void;
    /**
   ...
   * 跳转到口碑客领券详情页面
   */
  navigateToCouponDetail(r: {
    itemId: string;
    chInfo: string;
    /**
     * 接口调用成功的回调函数
     */
    success?(data: { [property: string]: unknown }): void;
    /**
     * 接口调用失败的回调函数
     */
    fail?(
  ...
  navigateToMiniService(r: {
    ...
    servicePage?: string;
    /**
     * 需要传递给目标插件的数据，插件可在 `App.onLaunch(options)`，`App.onShow(options)` 中获取到这份数据。
     */
    extraData?: Record<string, unknown>;
    /**
     * 接口调用成功的回调函数
    ...
    success?(data: {
      ...
      /**
       * 通过 `navigateBackFromMiniService` 传递的数据。
       */
      data: Record<string, unknown>;
    }): void;
    /**
  ...
  offFavorite(
    cb?: (arg: {
      /**
       * 触发事件的行为是否成功，`true` 为成功，`false` 为失败。
       */
      success: boolean;
      /**
       * 触发事件的行为，目前就添加收藏和取消收藏两种行为。
       */
   ...
   * 取消监听 mDNS 服务停止搜索的事件
   */
  offLocalServiceDiscoveryStop(cb?: () => void): void;
  /**
   * 取消监听 mDNS 服务发现的事件
   */
   ...
   */
  offSocketClose(cb?: (arg: { reason: string }) => void): void;
  /**
   * 取消监听 WebSocket 错误事件
   * @see https://opendocs.alipay.com/mini/api/kk7vv7
   */
  offSocketError(cb?: (arg: { errorMessage: string; error: number }) => void): void;
  /**
   ...
   * 取消监听标题栏关闭按钮点击事件
   */
  offTitleCloseClick(cb?: () => void): void;
  /**
   * 取消监听标题栏菜单点击事件
   */
  offTitleMoreClick(cb?: () => void): void;
  /**
   ...
   */
  offUserCaptureScreen(cb?: () => void): void;
  /**
   * 取消监听连接上 Wi-Fi 的事件
   ...
   */
  onSocketClose(cb: (arg: { reason: string }) => void): void;
  /**
   * 监听 WebSocket 错误事件
   * @see https://opendocs.alipay.com/mini/api/giu3c2
   */
  onSocketError(cb: (arg: { errorMessage: string; error: number }) => void): void;
  /**
   ...
   * 监听标题栏关闭按钮点击事件
   */
  onTitleCloseClick(cb: () => void): void;
  /**
   * 监听标题栏菜单点击事件
   */
  onTitleMoreClick(cb: () => void): void;
  /**
   * 监听未处理的 Promise 拒绝事件
   ...
   * 跳转到其他客户端
   */
  openOtherApplication(r: {
    /**
     * 跳转第三方 App 的完整scheme
     * @description 示例: "amapuri://route/plan/?sid=BGVIS1&slat=39.92848272"
     */
    url: string;
    /**
     * 接口调用成功的回调函数
  ...
  pauseBackgroundAudio(r?: {
     ...
     * 接口调用成功的回调函数
     */
    success?(data: { errorMessage: string; success: number }): void;
    /**
     * 接口调用失败的回调函数
     */
    fail?(
  ...
  playBackgroundAudio(r: {
     ...
     * 接口调用成功的回调函数
     */
    success?(data: { success: number; errorMessage: string }): void;
    /**
     * 接口调用失败的回调函数
     */
    fail?(err: { error?: number; errorMessage?: string }): void;
  ...
  playVoice(r: {
    /**
     * 音频文件路径，只能来源自 `startRecord` 的录音文件路径。
     */
    filePath: string;
    /**
     * 接口调用成功的回调函数
  ...
  preloadCanvasImage(r: {
    ...
    success?(data: {
       ...
       * 加载失败的图片
       */
      failed: IMyPreloadCanvasImageFailed;
      /**
       * 加载成功的图片
       */
      loaded: IMyPreloadCanvasImageLoaded;
    }): void;
    /**
     * 接口调用失败的回调函数
     */
  ...
  /**
   * 通过经纬度逆向获取地理信息
   * @deprecated 请使用 [my.getGeocoder]() 进行获取逆地理信息
   */
  reGeoCode(r: {
    /**
     * 业务唯一标示。
    ...
    success?(data: {
       ...
       * 街道信息。
       */
      streetNumber: IReGeoCodeStreetNumber;
      /**
       * poi 列表。
       */
      pois: IReGeoCodePois[];
   ...
   * 将当前小程序从收藏中移除。
   */
  removeFromFavorite(r?: {
    /**
     * 接口调用成功的回调函数
     */
    success?(data: {
      /**
       * 取消收藏是否成功，`true` 为添加收藏成功，`false` 为添加收藏失败。
       */
      success: boolean;
    }): void;
    /**
   ...
   */
  removeStorageSync(r: { key: string }): {
    success: true;
    error: 0;
  };
  /**
  ...
  /**
   */
  replyRoomInvitation(r: {
    params: IReplyRoomInvitationParams;
    uid: string;
    replyType: number;
    /**
     * 接口调用成功的回调函数
  ...
  /**
   * 上报自定义分析数据
   * @description 使用前需要在 [小程序开发者中心控制台](https://openhome.alipay.com/) > 小程序详情页 > 数据中心 > 自定义分析 > 事件 中新建事件，并配置事件名和字段。
   * @see https://opendocs.alipay.com/mini/api/report
   */
  ...
  /**
   * 上报自定义异常
   * @description 雨燕错误码为 14
   */
  reportCustomError(error: void | Error): void;
  /**
   * 上报小程序事件埋点
   * @description 事件管理相关请见 [此处](https://log.alipay.com/index_v3.htm#/manage/eventmanage/eventmanagement)
   */
  reportCustomEvent(eventId: string, extData: IMyReportCustomEventExtData): void;
  /**
   */
  reportIotEvent(r: {
    eventId: string;
   ...
   * 声明页面当前数据是否达到快照状态
   */
  reportSnapshot(param: {
    /**
     * 当前页面数据是否可被快照
     * @description
     * - false: 当前页面不会被快照存储（适用于页面进入异常状态）
     * - true: 当前页面最后一次 avaliable: true 会被记录作为快照存储
     */
    available: boolean;
  }): void;
  /**
  ...
  rpc(r: {
     ...
     * rpc 超时时间。
     * @description
     * - **警告**: 尽量不要自定义，否则容易出现服务端有成功返回数据，但客户端已经超时回调报错给前端的情况，容易导致重复发起创建订单之类的严重问题。
     * - 默认值: iOS端 wifi 环境 20s，其它环境 30s。Android端 wifi/4g 环境 12s 到 42s 之间。其它环境 32s 到 60s 之间
     * @native 10.0.3
     */
    timeout?: number;
    /**
     * 获取 rpc 响应头
     * @description **注意**: 如果设置为true的话，响应数据会多一层嵌套，可用于数据回流上报获取 traceId/entityId 场景，需要开启 MMTP 开关）
     * @native 10.0.18
     * @default false
     */
    getResponse?: boolean;
   ...
   * 发送局域网 HTTP 请求
   */
  sendLanHttpRequest(r: {
    /**
     * 网址，只支持局域网 IP 地址作为域名，不能是本机地址，只支持 `http` 或 `https`
     */
    url: string;
    /**
     * 设置请求的 HTTP 头对象，该对象里面的 `key` 和 `value` 必须是 `string` 类型
    ...
    method?: 'GET' | 'POST';
    /**
     * *,传给服务器的数据最终会被转换成 `string` 类型，转换规则如下：
    ...
    dataType?: 'JSON' | 'text';
    /**
     * [**Android Only**] 是否绑定到当前 WiFi，Android 默认不向无法上网的 WiFi 发送网络数据。
     * @default false
     */
    bindToWifi?: boolean;
    /**
  ...
  setBackgroundImage(r: {
    /**
     * 图片对应的链接，支持dataURL/相对路径/离线资源
     */
    imgUrl: string;
    /**
     * 图片的背景颜色，用于填补图片透明或空缺的位置
     */
    color?: number;
    /**
     * 接口调用成功的回调函数
  ...
  /**
   * @see https://opendocs.alipay.com/mini/00wjm3
   */
  setCustomPopMenu(r: {
    /**
     * 自定义菜单列表
     */
    menus: ISetCustomPopMenuMenus[];
    /**
     * 接口调用成功的回调函数
     */
    success?(data: { success: true }): void;
  ...
  /**
   * 设置导航栏菜单项配置
   * @see https://opendocs.alipay.com/mini/api/optionmenu
   */
  setOptionMenu(r?: {
    icon?: string;
    /**
   ...
   */
  setStorageSync(r: { data: unknown; key: string }): void;
  /**
   ...
   * 开始持续获取当前设备的经纬度信息
   */
  startContinuousLocation(r?: {
    /**
     * 业务方标识，要求能够唯一标识使用方的字符串。
     */
    bizType?: string;
    /**
     * 接口调用成功的回调函数
  ...
  /**
   * 打开小服务（插件 1.0）
   * @deprecated 小服务（插件 1.0）已不再维护
   * @description 从 `A` 主体小程序直接打开 `B` 主体小程序的 `b` 插件(1.0 小服务)。
   */
  startMiniService(r: {
    /**
     * 已订购的插件id。
  ...
  startSpeechRecognize(r?: {
    ...
    encodeAudio?: boolean;
    iOSKey?: string;
    extraInfo?: IStartSpeechExtraInfo;
    /**
     * 接口调用成功的回调函数
     */
    success?(data: { success: true }): void;
  ...
  startZMCreditRent(r: {
    ...
    /**
     * 类目（芝麻开发给到），暂时可以用：`ZMSC_2_1`
     */
    category: string;
    /**
     * 商品内容
     */
    subject: IMyApStartZMCreditRentSubject;
    /**
     * 逾期时间，`yyyy-MM-dd HH:mm:ss`，需要大于当前时间
     */
    overdue_time: string;
    /**
     * 该次支付总金额，单位为元，精确到小数点后两位，取值范围 `[0.01,100000000]`
     */
    amount: string;
    /**
     * 外部订单号，即商户自己的订单号
   ...
   * 停止持续获取当前设备的经纬度信息
   */
  stopContinuousLocation(r: {
    /**
     * 业务方标识，要求能够唯一标识使用方的字符串。需要和 `startContinuousLocation` 时的保持一致。
     */
    bizType: string;
    /**
     * 接口调用成功的回调函数
  ...
  textRiskIdentification(r: {
     ...
     * 识别类型。
     */
    type: ('0' | '1' | '2' | '3' | 'keyword')[];
    /**
     * 接口调用成功的回调函数
     */
    success?(data: { result: ITextRiskIdentificationResult }): void;
    /**
     * 接口调用失败的回调函数
     */
    fail?(err: { error?: number; errorMessage?: string }): void;
    ...
    complete?(
      arg:
        | {
            result: ITextRiskIdentificationResult;
          }
        | {
            error?: number;
            errorMessage?: string;
  ...
  zmRentTransition(r: {
    ...
    amount: string;
    /**
     * 押金；单位为元，精确到小数点后两位，取值范围 [0.01,100000000]。
     */
    deposit: string;
    /**
     * 芝麻订单号。
     ...
     */
    overdue_time: string;
    subject: IMyApZmRentTransitionSubject;
    /**
     * 接口调用成功的回调函数
     */
    success?(data: { [property: string]: unknown }): void;
...
export interface MyAPBridge {
  ...
  addAppToMyApps(r: {
    ...
    fail?(
      err:
        ...
        | {
            error: 60002;
            errorMessage: '当前应用不在白名单中或者疲劳度检测不通过';
          }
        | {
            error: 60003;
            errorMessage: '用户点击了取消';
          },
    ): void;
    /**
     * 接口调用结束的回调函数（调用成功、失败都会执行）
   ...
   * 创建 tracert 对象
   */
  createTracert(option: ITracertOptions): Tracert;
  /**
   * 刷脸认证
   */
  faceVerify(r: {
    /**
     * 业务流水号
     * @description 需要保证唯一性，不超过 64 位。
     */
    bizId: string;
    /**
     * 业务类型
     * @description 填写 2，代表刷脸认证场景。
     */
    bizType: string;
    /**
     * 接口调用成功的回调函数
  ...
  /**
   * 判断是否支付宝特别版
   * @deprecated 仅用于支付宝特别版
   */
  getSpecialEditionStateSync(): {
    /**
     * 是否支付宝特别版
     */
  ...
  /**
   */
  openCreditEvaluation(r: {
    /**
     * 信用评估单号
     * @description acceptanceId 与 signStr 不能同时为空
     */
    creditRequestOrderId?: string;
    /**
     * 如果 creditRequestOrderId是评估单号，则为 'true'
     */
    isAcceptanceId: string;
    /**
     * 签约字符串
     * @description acceptanceId 与 signStr 不能同时为空
     */
  ...
  showFuCard(r: {
    ...
    success?(data: {
       ...
       * 福卡 toast 消失类型
       */
      toastViewDismiss: `${EAlipayNewYearNebulaPluginShowFuCardToastViewDimiss}`;
      /**
       * 收取福卡结果
       * - true: 表示点击收下福卡
       * - false: 表示点击关闭弹窗
...
declare namespace NFCAdapter {
  ...
  export interface IReadNdefMessageData {
    messages: IReadNdefMessageDataMessages[];
  }
  export interface IReadNdefMessageDataMessages {
    records: IReadNdefMessageDataMessagesRecords[];
  }
  export interface IReadNdefMessageDataMessagesRecords {
    tnf: string;
    id: string;
    type: string;
    payload: string;
  }
  export interface IWriteNdefMessageRecords {
    tnf: string;
    id: string;
    type: string;
    payload: string;
...
export interface SelectorQuery {
  ...
  selectViewport(): this;
  /**
   * @notice 本 API 为不稳定的实验性功能，请谨慎用于生产环境，并保证必要的回归测试。
   * 将当前选择的节点生成为指定格式的图片。
   * @description 支持图片生成的基础组件与样式详见：[SelectorQuery.toImage](https://yuque.antfin.com/appxng/open-docs/to-image)
   */
  toImage(options: ISelectorQueryToImageOptions, cb: (res: ISelectorQueryToImageCallbackResult) => void): this;
}
...
export interface CameraContext {
  /**
   * 获取实时帧数据
   * @see https://opendocs.alipay.com/mini/03qitt
   */
  onCameraFrame(callback: (payload: ICameraContextOnCameraFrameCallback) => void): CameraFrameListener;
  /**
   * 设置摄像头缩放级别
   * @see https://opendocs.alipay.com/mini/03qde9
   */
  setZoom(r: {
    /**
     * 缩放级别
    ...
    fail?(
      err:
        ...
        | {
            error: 4;
            errorMessage: '无权调用 CameraContext.setZoom';
          },
    ): void;
    /**
     * 接口调用结束的回调函数（调用成功、失败都会执行）
  ...
  /**
   * 开始摄像头录像
   * @see https://opendocs.alipay.com/mini/03qits
   */
  startRecord(r?: {
    /**
     * 录像自动结束回调
     */
    timeoutCallback?: (payload: ICameraContextStartRecordTimeoutCallback) => void;
    /**
     * 最长录制时间
     */
    maxDuration?: number;
    ...
    fail?(
      err:
        ...
        | {
            error: 4;
            errorMessage: '\u65E0\u6743\u8C03\u7528CameraContext.startRecord';
          }
        | {
            error: 7;
            errorMessage: '已在录像中';
  ...
  /**
   * 结束摄像头录像
   * @see https://opendocs.alipay.com/mini/03qfj8
   */
  stopRecord(r?: {
    /**
     * 接口调用成功的回调函数
    ...
    fail?(
      err:
        ...
        | {
            error: 4;
            errorMessage: '\u65E0\u6743\u8C03\u7528CameraContext.stopRecord';
          },
    ): void;
    /**
     * 接口调用结束的回调函数（调用成功、失败都会执行）
  ...
  /**
   * 拍摄照片
   * @see https://opendocs.alipay.com/mini/03qde8
   */
  takePhoto(r?: {
    /**
     * 成像质量
    ...
    fail?(
      err:
        ...
        | {
            error: 4;
            errorMessage: '\u65E0\u6743\u8C03\u7528CameraContext.takePhoto';
          },
    ): void;
    /**
     * 接口调用结束的回调函数（调用成功、失败都会执行）
...
export interface CameraFrameListener {
  /**
   * 开始监听摄像头帧数据
   * @see https://opendocs.alipay.com/mini/03qfja
   */
  start(r?: {
    /**
     * 接口调用成功的回调函数
     */
    success?(data: { error: 4; errorMessage: '\u65E0\u6743\u8C03\u7528CameraFrameListener.start' }): void;
    /**
     * 接口调用失败的回调函数
     */
    fail?(err: { error?: number; errorMessage?: string }): void;
    ...
    complete?(
      arg:
        | {
            error: 4;
            errorMessage: '\u65E0\u6743\u8C03\u7528CameraFrameListener.start';
          }
        | {
            error?: number;
            errorMessage?: string;
  ...
  /**
   * 停止监听摄像头帧数据
   * @see https://opendocs.alipay.com/mini/03qdea
   */
  stop(r?: {
    /**
     * 接口调用成功的回调函数
     */
    success?(data: { error: 4; errorMessage: '\u65E0\u6743\u8C03\u7528CameraFrameListener.stop' }): void;
    /**
     * 接口调用失败的回调函数
     */
    fail?(err: { error?: number; errorMessage?: string }): void;
    ...
    complete?(
      arg:
        | {
            error: 4;
            errorMessage: '\u65E0\u6743\u8C03\u7528CameraFrameListener.stop';
          }
        | {
            error?: number;
            errorMessage?: string;
...
export interface Worker {
   ...
   * 取消监听来自 Worker 线程的消息
   */
  offMessage(cb?: (arg: { workerId: string; message: Record<string, unknown> }) => void): void;
  /**
   * 监听来自 Worker 线程的消息
   * @see https://opendocs.alipay.com/mini/api/workeronmesssage
   */
  onMessage(cb: (arg: { workerId: string; message: Record<string, unknown> }) => void): void;
  /**
   * 向 Worker 线程发送消息
...
export interface TCPSocket {
   ...
   * 取消监听给定套接字连接关闭的事件
   */
  offClose(cb?: (arg: {}) => void): void;
  /**
   * 取消监听给定套接字连接成功建立的事件
   */
  offConnect(cb?: (arg: { socketId: string }) => void): void;
  /**
   * 取消监听给定套接字连接异常的事件
   */
  offError(cb?: (arg: {}) => void): void;
  /**
   * 取消监听给定套接字连接消息的事件
   */
  offMessage(cb?: (arg: {}) => void): void;
  /**
   * 监听给定套接字连接关闭的事件
   */
  onClose(cb: (arg: {}) => void): void;
  /**
   * 监听给定套接字连接成功建立的事件
   */
  onConnect(cb: (arg: { socketId: string }) => void): void;
  /**
   * 监听给定套接字连接异常的事件
   */
  onError(cb: (arg: {}) => void): void;
  /**
   * 监听给定套接字连接消息的事件
   */
  onMessage(cb: (arg: {}) => void): void;
  /**
   * 向给定的套接字连接发送消息
   */
  send(param: {
...
export interface AICameraContext {
  ...
  startVideoRecognize(r: {
     ...
     * 拓展参数
     */
    options?: IAICameraContextStartVideoRecognizeOptions;
    /**
     * 接口调用成功的回调函数
     */
    success?(data: { success: true }): void;
...
export interface ARContext {
  /**
   * 绑定替换节点。
   * @description 设置了替换节点后，引导平面的UI绘制将被节点渲染替代。
   */
  bindGuidePlaneNodeId(param: {
    /**
     * 节点 id。
     */
    nodeId: string;
   ...
   * 清除 AR 动画叠加效果
   */
  clearAnimationAdditiveBlending(param: {
    /**
     * 目标节点 id。
     */
    targetNodeId: string;
  ...
  createAnimation(param: {
     ...
     * 动画类型标识。
     */
    propertyId: EARContextCreateAnimationPropertyId;
    /**
     * 关键帧的帧数。
     */
    keyCount: number;
     ...
     * 插值类型。
     */
    type: EARContextCreateAnimationType;
  }): void;
  /**
   * 基于已有动画创建一个片段。
   */
  createAnimationClip(param: {
    /**
     * 动画片段 id。
     */
    clipId: string;
   ...
   * 动画淡入淡出效果。
   * @description
   * 从 srcClipId 过渡到 dstClipId，持续时间为 duration，
   * 如果 srcClipId 没有在播放，则先播放它再开始淡入淡出效果。
   */
  crossFadeAnimation(param: {
    /**
     * 源动画片段，
     */
    srcClipId: string;
  ...
  getFaceTrackingStatus(r?: {
    ...
    success?(data: {
       ...
       * 引擎状态。
       */
      status: 'NotSupport' | 'Initializing' | 'Ready' | 'Unknow';
    }): void;
    /**
     * 接口调用失败的回调函数
     */
   ...
   * 获取指定手势的得分。
   */
  getGestureDetectingScore(r: {
    /**
     * 姿态 id。
     */
    gestureId: 'Gesture-b45' | 'Gesture-f45' | 'Gesture-fAll' | 'Gesture-bAll' | 'Gesture-f34' | 'Gesture-b34' | 'Gesture-f0' | 'Gesture-d0' | 'Gesture-b15' | 'Gesture-f15';
    /**
     * 接口调用成功的回调函数
     */
    success?(data: {
  ...
  getNodeScale(r: {
     ...
     * 接口调用成功的回调函数
     */
    success?(data: { x: number; y: number; z: number }): void;
    /**
     * 接口调用失败的回调函数
     */
    fail?(err: { error?: number; errorMessage?: string }): void;
   ...
   * 获取指定姿态的得分。
   */
  getPoseDetectingScore(r: {
    /**
     * 姿态 id。
     */
    poseId: number;
    /**
     * 是否需要返回姿态结果数据，包含关键点信息。
     */
    needResult?: boolean;
    /**
     * 接口调用成功的回调函数
  ...
  getTrackMode(r?: {
     ...
     * 接口调用成功的回调函数
     */
    success?(data: { trackMode: string }): void;
    /**
     * 接口调用失败的回调函数
     */
    fail?(err: { error?: number; errorMessage?: string }): void;
   ...
   * 加载动画。
   * @description
   * 动态的加载动画，对于已有的模型，其中包含了一些骨骼节点，
   * 通过此API可以动态的加载离线制作的动画数据，而不需要去更新模型。
   */
  loadAnimation(param: {
    /**
     * 动画文件路径。
  ...
  modulateColor(param: {
    nodeId: string;
    /**
     * 颜色 r 分量调整系数，范围：0.0～1.0。
     */
    r: number;
    /**
     * 颜色 g 分量调整系数，范围：0.0～1.0。
     */
    g: number;
    /**
     * 颜色 b 分量调整系数，范围：0.0～1.0。
     */
    b: number;
    /**
     * 贴图序号(通常为0)。
  ...
  /**
   * 移除贴图缓存
   * @description 减少内存压力，不再使用的贴图需即时移除
   */
  removeTextureCache(param: {
    /**
     * 贴图文件路径。
     */
    imgPath: string;
  ...
  /**
   * 设置 AR 动画叠加效果
   * @description 在已有动画的基础上，对某个骨骼节点进行调整，调整后的动作与动画叠加播放。
   */
  setAnimationAdditiveBlending(param: {
    /**
     * 目标节点 id。
     */
    targetNodeId: string;
     ...
     * 动画类型标识。
     */
    propertyId: EARContextSetAnimationAdditiveBlendingPropertyId;
    /**
     * 帧属性值数组。
     */
    keyValues: number[];
  ...
  /**
   * 改变环境光颜色 (需要在.scene文件中开启)。
   */
  setLightAmbientColor(param: {
    /**
     * 颜色 r 分量。
     */
    r: number;
  ...
  /**
   * 设置指定节点为billboard效果(始终面向相机)
   */
  setNodeBillboard(param: {
    /**
     * 节点 id。
     */
    nodeId: string;
  ...
  /**
   * 设置指定节点为static效果(与相机保持固定位置关系(旋转、平移、缩放))。
   */
  setNodeStatic(param: {
    /**
     * 节点 id。
  ...
  /**
   * 开始更新 AR 环境纹理
   * @description 使视频预览绑定至模型节点，实现与实景融合渲染的效果
   */
  startEnvironmentTexturing(param: {
    /**
     * 节点id
     */
    nodeId: string[];
   ...
   * 开启手势识别。
   */
  startGestureDetecting(): void;
  /**
   * 开始图片识别
   */
  startImageDetecting(param: {
    /**
     * json 字符串，包含字段。
     */
    option?: string;
  ...
  /**
   * 停止更新 AR 环境纹理
   * @description 使节点的纹理更新停留在这一帧。
   */
  stopEnvironmentTexturing(param: {
    /**
     * 节点id
     */
    nodeId: string[];
   ...
   * 停止手势识别。
   */
  stopGestureDetecting(): void;
  /**
   * 停止图片识别。
   */
  stopImageDetecting(): void;
  /**
   * 停止姿态识别。
   */
  stopPoseDetecting(): void;
  /**
   * 停止录制视频。
   */
  stopRecording(): void;
  ...
  transformCoordinate(r: {
    ...
    y: number;
    /**
     * 坐标 z 分量（若 flag 为 false，z 为 depth）。
     */
    z: number;
    /**
     * 标识位，true: 世界坐标 > 屏幕坐标，false: 屏幕坐标 > 世界坐标。
     */
    flag: boolean;
    /**
     * 接口调用成功的回调函数
    ...
    success?(data: {
      ...
      y: number;
      /**
       * 坐标 z 分量（若 flag 为 true，z 为 depth）。
       */
      z: number;
    }): void;
    /**
...
export interface InsuranceXReplaySession {
  ...
  init(): void;
  /**
   * 保险屏幕操作录制会话连接到当前页面的回调
   */
  onPageConnect(page: any, payload: any): any;
}
...
export interface LivePlayerContext {
  ...
  /**
   * 播放
   * @description autoplay=true 时可以不调用，否则调用 play 才开始播放。
   */
  play(): void;
  /**
   * 进入全屏
   * @description 支持3个屏幕方向
   */
...
export interface XnnManager {
  /**
   */
  inference(param: { inputTensors: any; outputTensors: any; success: (payload: IXnnManagerInferenceSuccess) => void; fail: () => void }): void;
  /**
   */
  load(param: { modelPath: string; aesKey?: string; license?: string; success: (payload: IXnnManagerLoadSuccess) => void; fail: () => void }): void;
  /**
   */
  release(param: { success: () => void; fail: () => void }): void;
  /**
   */
  transformFromImage(param: { data: ArrayBuffer; options: any; success: (payload: IXnnManagerTransformFromImageSuccess) => void; fail: () => void }): void;
  /**
   */
  transformFromVideoFrame(param: {
    data: ArrayBuffer;
    width: number;
    height: number;
    format: string;
    options: any;
    success: (payload: IXnnManagerTransformFromVideoFrameSuccess) => void;
    fail: () => void;
  }): void;
}
...
export interface RtcRoomContext {
   ...
   */
  audioMode(r: {
    mode: ERtcRoomContextAudioModeMode;
    /**
     * 接口调用成功的回调函数
     */
    success?(data: {}): void;
...
export const enum EAICameraContextStartVideoRecognizeOptionsChannel {
  /**
   * 端处理
   */
  _0 = 0,
...
export const enum EAICameraContextStartVideoRecognizeOptionsLifecycle {
  /**
   * 引擎正常生命周期，init 创建，release 销毁
   */
  _0 = 0,
  /**
   * 引擎智能销毁模式，run 接口创建，进入后台或空闲销毁
   */
  _1 = 1,
  /**
   * 引擎即刻销毁模式，run 接口创建、处理、销毁引擎
   */
  _2 = 2,
}

export const enum EAICameraContextStartVideoRecognizeOptionsStandard {
  /**
   * 返回非标准化格式结果 JSON
   */
  _0 = 0,
...
export const enum EARContextCreateAnimationPropertyId {
  /**
   * 整体缩放。
   */
  _0 = 0,
...
export const enum EARContextCreateAnimationType {}

export const enum EARContextSetAnimationAdditiveBlendingPropertyId {}

export const enum EActionSheetBadgesType {
  /**
   * 显示文本
   */
  text = 'text',
...
export const enum EAddPeripheralServiceCharacteristicsProperties {
  /**
   * 可写
   */
  write = 'write',
...
export const enum EAlipayNewYearNebulaPluginShowFuCardToastViewDimiss {
  /**
   * 表示点击跳转消失
   */
  timeDismiss = 'timeDismiss',
  /**
   * 表示到时间自动消失
   */
  jumpDismiss = 'jumpDismiss',
}

export const enum EBeaconUpdateBeaconsProximity {
  /**
   * 未知
   */
  _0 = 0,
...
export const enum ECalculateRouteMode {
  /**
   * 最快捷模式，速度优先（时间）。
   */
  _0 = 0,
...
export const enum EChooseDistrictHeadItemType {
  /**
   * 常规城市
   */
  _0 = 0,
...
export const enum EChooseDistrictMode {
  /**
   * 境内
   */
  _0 = 0,
...
export const enum EChooseImageSizeType {
  /**
   * 原图
   */
  original = 'original',
...
export const enum EChooseImageSourceType {
  /**
   * 拍照
   */
  camera = 'camera',
...
export const enum EChooseVideoCamera {
  /**
   * 默认拉起后置摄像头。
   */
  back = 'back',
...
export const enum EChooseVideoSourceType {
  /**
   * 从相册选择视频。
   */
  album = 'album',
...
export const enum ECompressImageCompressLevel {
  /**
   * 低质量
   */
  _0 = 0,
...
export const enum EDepositState {
  /**
   * 支持
   */
  Y = 'Y',
...
export const enum EFileSystemEncoding {
  ascii = 'ascii',
  base64 = 'base64',
  hex = 'hex',
  binary = 'binary',
...
export const enum EGetAuthCodeScopeNicks {
  /**
   * 静默授权。
   */
  auth_base = 'auth_base',
...
export const enum EGetAuthorizeScopeNicks {
  /**
   * 静默授权。
   */
  auth_base = 'auth_base',
...
export const enum EGetBLEDeviceStatusAuthStatus {
  /**
...
export const enum EGetBLEDeviceStatusPowerStatus {
  /**
   * 表示设备不支持
   */
  _2 = 2,
...
export const enum EGetNetworkTypeNetworkInfo {
  /**
   * 未知
   */
  UNKNOWN = 'UNKNOWN',
...
export const enum EGetVideoInfoOrientation {
  /**
   * 默认
   */
  _0 = 0,
...
export const enum EMemoryWarningLevel {
  /**
   * 低内存告警
   */
  _10 = 10,
...
export const enum EMyGetLocationType {
  /**
   * 获取经纬度
   */
  _0 = 0,
...
export const enum ERentSettleType {
  /**
...
export const enum ERtcRoomContextAudioModeMode {}

export const enum EShowAuthGuideAuthType {
  /**
   * 蓝牙
   */
  BLUETOOTH = 'BLUETOOTH',
...
export const enum ESpuName {
  'power_bank' = 'power_bank',
  'umbrella' = 'umbrella',
}

export interface IAICameraContextStartVideoRecognizeOptions {
  /**
   * 算法是否需要降级
   * @default 0
   */
   ...
   * 算法模型对应云控开关 key 值
   */
  modelCloudKey?: string;
  /**
   * 引擎模式
   */
  lifecycle?: EAICameraContextStartVideoRecognizeOptionsLifecycle;
  /**
   * 可选服务链路
   */
  channel?: EAICameraContextStartVideoRecognizeOptionsChannel;
  /**
   * 端侧返回结果格式
   */
  standard?: EAICameraContextStartVideoRecognizeOptionsStandard;
}

/**
 * 基金下单页
 */
export interface IAP$NavigateToFinanceOptionsFundBuy {
  /**
   * 跳转类型，固定为 fundBuy
   */
  type: 'fundBuy';
 ...
 * 基金详情页
 */
export interface IAP$NavigateToFinanceOptionsFundDetail {
  /**
   * 跳转类型，固定为 fundDetail
   */
  type: 'fundDetail';
 ...
 * 基金定投设置页
 */
export interface IAP$NavigateToFinanceOptionsFundSign {
  /**
   * 跳转类型，固定为 fundSign
   */
  type: 'fundSign';
 ...
 * 黄金下单页
 */
export interface IAP$NavigateToFinanceOptionsGoldBuy {
  /**
   * 跳转类型，固定为 goldBuy
   */
  type: 'goldBuy';
 ...
 * 基金定投设置页
 */
export interface IAP$NavigateToFinanceOptionsGoldSign {
  /**
   * 跳转类型，固定为 goldSign
   */
  type: 'goldSign';
 ...
 * 跳转 H5页面
 */
export interface IAP$NavigateToFinanceOptionsH5Page {
  /**
   * 跳转类型，固定为 h5Page
   */
  type: 'h5Page';
 ...
 * 基金组合下单页
 */
export interface IAP$NavigateToFinanceOptionsPortfolioBuy {
  /**
 ...
 * 组合基金详情页
 */
export interface IAP$NavigateToFinanceOptionsPortfolioDetail {
  /**
 ...
 * 财富号首页
 */
export interface IAP$NavigateToFinanceOptionsWealthShop {
  /**
   * 跳转类型，固定为 wealthShop
   */
  type: 'wealthShop';
...
export interface IActionSheetBadges {
  /**
   * 需要飘红的选项的索引，从 0 开始
   */
  index: number;
...
export interface IAddPeripheralServiceCharacteristics {
  /**
   * 特征值 id。
   */
  id: string;
   ...
   * 特征值属性。
   */
  properties: Array<`${EAddPeripheralServiceCharacteristicsProperties}`>;
}

export interface IAppendFileRequest {
  /**
   * 要追加内容的文件路径
   */
  filePath: string;
...
export interface IBeaconUpdateBeacons {
  /**
   * iBeacon 设备广播的 UUID。
   */
  uuid: string;
...
export interface IBeehiveMultilevelSelectList {
  /**
   * 条目名称
   */
  name: string;
...
export interface IBeehiveMultilevelSelectResult {
  /**
   * 上一次选择的子条目
   */
  name: string;
...
export interface ICalculateRouteThroughPoints {
  /**
   * 纬度。
   */
  lat: number;
...
export interface ICameraContextOnCameraFrameCallback {
  /**
   * 图像数据矩形的宽度
   */
  width: number;
...
export interface ICameraContextStartRecordTimeoutCallback {
  /**
   * 封面图片文件的临时路径
   */
  tempThumbPath: string;
...
export interface IChooseDistrictHeadItem {
  /**
   * 区块名，如“热门城市”。
   */
  title: string;
...
export interface IChooseDistrictListItem {
  /**
   * 城市名
   */
  name: string;
...
export interface IChooseDistrictSeniorPageList {
  /**
...
export interface IGetBLEDeviceServicesServices {
  /**
   * 蓝牙设备特征值对应服务的 UUID。
   */
  serviceId: string;
...
export interface IGetBeaconsBeacons {
  /**
   * iBeacon 信号强度
...
export interface IGetConnectedWifiWifi {
  /**
   * wifi SSID信息
   */
  SSID: string;
...
export interface IGetFileInfoRequest {
  /**
   * 文件路径
   */
  filePath: string;
...
export interface IGetFileInfoResponse {
  /**
   * 文件大小
   * @description 以字节为单位
   */
...
export interface IGetSavedFileListFileList {
  size: number;
  apFilePath: string;
  createTime: number;
}

export interface IGetSavedFileListResponse {
  /**
   * 文件数组
   */
  fileList: Array<{
...
export interface IGetSettingAuthSetting {
  /**
   * 地理位置。
   */
  location?: boolean;
...
export interface IGetWifiListWifiList {
  /**
   * Wifi 的 SSID。
   */
  SSID: string;
...
export interface IIntersectionObserverObserveCallback {
  /**
   * 相交比例
   */
...
export interface IIntersectionObserverObserveCallbackBoundingClientRect {
  /**
   * 上边界
   */
  top: number;
...
export interface IIntersectionObserverObserveCallbackIntersectionRect {
  /**
   * 上边界
   */
  top: number;
...
export interface IIntersectionObserverObserveCallbackRelativeRect {
  /**
   * 上边界
   */
  top: number;
...
export interface IIntersectionObserverRelativeToMargins {
  /**
   * 节点布局区域的上边界
   */
  top?: number;
...
export interface IMapContextAddGroundOverlayIncludePoints {
  /**
   * 纬度
   */
  latitude: number;
...
export interface IMapContextAddMarkersMarkers {
  /**
   * 标记点 id
...
export interface IMapContextAddMarkersMarkersCallout {
  /**
   * 内容。
   */
  content?: string;
...
export interface IMapContextAddMarkersMarkersCustomCallout {
  /**
   * 样式类型。
   * @description
   * 有效值如下：
...
export interface IMapContextAddMarkersMarkersCustomCalloutDescList {
  /**
   * 文案
   */
  desc: string;
...
export interface IMapContextAddMarkersMarkersCustomCalloutLayout {}

export interface IMapContextAddMarkersMarkersDisplayRanges {
  /**
   * 缩放级别下边界
   */
  from: number;
...
export interface IMapContextAddMarkersMarkersFixedPoint {
  /**
   * 横向像素点
...
export interface IMapContextAddMarkersMarkersIconLayout {}

export interface IMapContextAddMarkersMarkersLabel {
  /**
   * 文案
   */
  content: string;
...
export interface IMapContextAddMarkersMarkersStyle {
  /**
   * 类型
   */
  type: 1 | 2 | 3;
...
export interface IMapContextCalculateDistancePoints {
  /**
   * 纬度。
   */
  latitude: number;
...
export interface IMapContextCalculateDistanceTargetPoints {
  /**
 ...
 * 自定义标记点上方的气泡窗口
 */
export interface IMapContextCallout {
  /**
   * 内容。
   */
  content?: string;
...
export interface IMapContextChangeMarkersAdd {
  /**
   * 标记点 id
   * @description 标记点 id，点击事件回调会返回此 id。
   */
...
export interface IMapContextChangeMarkersAddCallout {
  /**
   * 内容。
   */
  content?: string;
...
export interface IMapContextChangeMarkersAddCustomCallout {
  /**
   * 样式类型。
   * @description
   * 有效值如下：
...
export interface IMapContextChangeMarkersAddCustomCalloutDescList {
  /**
   * 文案
   */
  desc: string;
...
export interface IMapContextChangeMarkersAddCustomCalloutLayout {}

export interface IMapContextChangeMarkersAddDisplayRanges {
  /**
   * 缩放级别下边界
   */
  from: number;
...
export interface IMapContextChangeMarkersAddFixedPoint {
  /**
   * 横向像素点
   * @description 距离地图左上角的像素数值，从 `0` 开始。
   */
...
export interface IMapContextChangeMarkersAddIconLayout {}

export interface IMapContextChangeMarkersAddLabel {
  /**
   * 文案
   */
  content: string;
...
export interface IMapContextChangeMarkersAddStyle {
  /**
   * 类型
   */
  type: 1 | 2 | 3;
...
export interface IMapContextChangeMarkersRemove {
  /**
   * 标记点 id
   * @description 标记点 id，点击事件回调会返回此 id。
   */
...
export interface IMapContextChangeMarkersRemoveCallout {
  /**
   * 内容。
   */
  content?: string;
...
export interface IMapContextChangeMarkersRemoveCustomCallout {
  /**
   * 样式类型。
   * @description
   * 有效值如下：
...
export interface IMapContextChangeMarkersRemoveCustomCalloutDescList {
  /**
   * 文案
   */
  desc: string;
...
export interface IMapContextChangeMarkersRemoveCustomCalloutLayout {}

export interface IMapContextChangeMarkersRemoveDisplayRanges {
  /**
   * 缩放级别下边界
   */
  from: number;
...
export interface IMapContextChangeMarkersRemoveFixedPoint {
  /**
   * 横向像素点
   * @description 距离地图左上角的像素数值，从 `0` 开始。
   */
...
export interface IMapContextChangeMarkersRemoveIconLayout {}

export interface IMapContextChangeMarkersRemoveLabel {
  /**
   * 文案
   */
  content: string;
...
export interface IMapContextChangeMarkersRemoveStyle {
  /**
   * 类型
   */
  type: 1 | 2 | 3;
...
export interface IMapContextChangeMarkersUpdate {
  /**
   * 标记点 id
   * @description 标记点 id，点击事件回调会返回此 id。
   */
...
export interface IMapContextChangeMarkersUpdateCallout {
  /**
   * 内容。
   */
  content?: string;
...
export interface IMapContextChangeMarkersUpdateCustomCallout {
  /**
   * 样式类型。
   * @description
   * 有效值如下：
...
export interface IMapContextChangeMarkersUpdateCustomCalloutDescList {
  /**
   * 文案
   */
  desc: string;
...
export interface IMapContextChangeMarkersUpdateCustomCalloutLayout {}

export interface IMapContextChangeMarkersUpdateDisplayRanges {
  /**
   * 缩放级别下边界
   */
  from: number;
...
export interface IMapContextChangeMarkersUpdateFixedPoint {
  /**
   * 横向像素点
   * @description 距离地图左上角的像素数值，从 `0` 开始。
   */
...
export interface IMapContextChangeMarkersUpdateIconLayout {}

export interface IMapContextChangeMarkersUpdateLabel {
  /**
   * 文案
   */
  content: string;
...
export interface IMapContextChangeMarkersUpdateStyle {
  /**
   * 类型
   */
  type: 1 | 2 | 3;
 ...
 * 自定义 callout 背景
 */
export interface IMapContextCustomCallout {
  /**
   * 样式类型。
   * @description 有效值如下：
   * - `0` 为黑色 style
...
export interface IMapContextCustomCalloutDesc {
  /**
   * 文案
   */
  desc: string;
...
export interface IMapContextDisplayRange {
  /**
   * 缩放级别下边界
   */
  from: number;
...
export interface IMapContextIncludePointsPoints {
  /**
   * 纬度
   */
  latitude: number;
 ...
 */
export interface IMapContextMarker {
  /**
   * 标记点 id
   * @description 标记点 id，点击事件回调会返回此 id。
   */
 ...
 * 基于屏幕位置的扎点。
 */
export interface IMapContextMarkerFixedPoint {
  /**
   * 横向像素点
   * @description 距离地图左上角的像素数值，从 `0` 开始。
   */
...
export interface IMapContextMarkerLabel {
  /**
   * 文案
   */
  content: string;
...
export interface IMapContextMarkerStyle {
  /**
   * 类型
   */
  type: 1 | 2 | 3;
 ...
 * 经纬度点位。
 */
export interface IMapContextPoint {
  /**
   * 纬度
   */
  latitude: number;
...
export interface IMapContextPolygonContainsPointPoint {
  /**
   * 纬度
   */
  latitude: number;
...
export interface IMapContextPolygonContainsPointPolygon {
  /**
   * 纬度
   */
  latitude: number;
...
export interface IMapContextSmoothMovePolylinePoints {
  /**
   * 纬度
   */
  latitude: number;
...
export interface IMapContextUpdateComponentsCommand {
  markerAnim?: IMapContextUpdateComponentsCommandMarkerAnim;
}

export interface IMapContextUpdateComponentsCommandMarkerAnim {
  type: number;
  markerId: number;
}

export interface IMapContextUpdateComponentsIncludePadding {
  left: number;
  right: number;
  top: number;
  bottom: number;
...
export interface IMapContextUpdateComponentsIncludePoints {
  /**
   * 纬度
   */
  latitude: number;
...
export interface IMapContextUpdateComponentsMarkers {
  /**
   * 标记点 id
   * @description 标记点 id，点击事件回调会返回此 id。
   */
...
export interface IMapContextUpdateComponentsMarkersCallout {
  /**
   * 内容。
   */
  content?: string;
...
export interface IMapContextUpdateComponentsMarkersCustomCallout {
  /**
   * 样式类型。
   * @description
   * 有效值如下：
...
export interface IMapContextUpdateComponentsMarkersCustomCalloutDescList {
  /**
   * 文案
   */
  desc: string;
...
export interface IMapContextUpdateComponentsMarkersCustomCalloutLayout {}

export interface IMapContextUpdateComponentsMarkersDisplayRanges {
  /**
   * 缩放级别下边界
   */
  from: number;
...
export interface IMapContextUpdateComponentsMarkersFixedPoint {
  /**
   * 横向像素点
   * @description 距离地图左上角的像素数值，从 `0` 开始。
   */
...
export interface IMapContextUpdateComponentsMarkersIconLayout {}

export interface IMapContextUpdateComponentsMarkersLabel {
  /**
   * 文案
   */
  content: string;
...
export interface IMapContextUpdateComponentsMarkersStyle {
  /**
   * 类型
   */
  type: 1 | 2 | 3;
...
export interface IMapContextUpdateComponentsPolyline {
  /**
   * 路线的经纬度集合
   */
...
export interface IMapContextUpdateComponentsPolylinePoints {
  /**
   * 纬度
   */
  latitude: number;
...
export interface IMapContextUpdateComponentsSetting {
  gestureEnable?: 0 | 1;
  showCompass?: 0 | 1;
  showScale?: 0 | 1;
...
export interface IMapContextUpdateComponentsSettingLogoPosition {
  centerX?: number;
  centerY?: number;
}

export interface IMapContextUpdateGroundOverlayIncludePoints {
  /**
   * 纬度
   */
  latitude: number;
...
export interface IMkdirRequest {
  /**
   * 创建的目录路径
   */
  dirPath: string;
...
export interface IMyApAuthorizeAuthErrorScopes {
  /**
...
export interface IMyApGetAuthCodeAuthErrorScopes {
  /**
   * key 是授权失败的 scope，value 是对应的错误码
   */
  [scope: string]: string;
...
export interface IMyApNsfBizContext {
  /**
...
export interface IMyApNsfResult {
  /**
   * 风险识别结果
   * @description
...
export interface IMyApPreventCheatBizContext {
  /**
   * 合作伙伴匹配服务类型,请按照示例值填写
...
export interface IMyApPreventCheatResult {
  /**
   * 风险识别结果
   * @description
   * 示例值：
...
export interface IMyApStartZMCreditRentSubject {
  products: IMyApStartZMCreditRentSubjectProducts[];
}

export interface IMyApStartZMCreditRentSubjectProducts {
  amount: string;
  count: number;
  name: string;
}

export interface IMyApZmRentTransitionSubject {
  products: IMyApZmRentTransitionSubjectProducts[];
}

export interface IMyApZmRentTransitionSubjectProducts {
  amount: string;
  count: number;
  name: string;
}

export interface IMyChooseAlipayContactContacts {
  /**
   * 账号的真实姓名。
   */
  realName: string;
...
export interface IMyChooseContactContactsDicArray {
  /**
   * 支付宝账号唯一标识符。
   */
  userId: string;
...
export interface IMyCreateIntersectionObserverOption {
  thresholds?: number[];
  initialRatio?: number;
  selectAll?: boolean;
  dataset?: boolean;
...
export interface IMyCreateRDSContextOption {
  appId: string;
}

export interface IMyCreateSelectorQueryOption {
  /**
   * 是否开启选择器严格模式
   * @sdk 2.7.3
   */
...
export interface IMyGetAccountInfoSyncMiniProgram {
  /**
   * 小程序 appId
   */
  appId: string;
...
export interface IMyGetAccountInfoSyncPlugin {
  /**
   * 插件 appId
   */
  appId: string;
...
export interface IMyGetAppBaseInfoHost {
  /**
   * 宿主 app 对应的 appId。
   */
  appId: string;
...
export interface IMyGetLocationPois {
  /**
   * poi名称。
   */
  number: string;
...
export interface IMyGetLocationStreetNumber {
  /**
   * 街道 number。
   */
  number: string;
...
export interface IMyGetWindowInfoSafeArea {
  left: number;
  right: number;
  top: number;
  bottom: number;
...
export interface IMyLoadFontFaceDesc {
  /**
   * 字体样式
...
export interface IMyNavigateToEvents {
  /**
   * 特定事件名监听回调
   */
...
export interface IMyNavigateToMiniProgramExtraData {
  /**
   * 参数键值对
   */
  [key: string]: unknown;
...
export interface IMyNavigateToMiniProgramQuery {
  /**
   * query 参数键值对
   */
  [key: string]: unknown;
...
export interface IMyPreloadCanvasImageFailed {
  /**
   * 图片信息
   */
  [url: string]: CanvasContextImageSource;
}

export interface IMyPreloadCanvasImageLoaded {
  /**
   * 图片信息
   */
  [url: string]: CanvasContextImageSource;
...
export interface IMyReportCustomEventExtData {
  bizType?: string;
  /**
   * 上报日志级别，仅支持 1 | 2 | 3
   * - 1：日志等级 High
...
export interface IMyRequestSubscribeMessageResult {
  /**
...
export interface IOpenEmbeddedMiniProgramParam {
  [key: string]: unknown;
}

export interface IOpenSettingAuthSetting {
  /**
   * 地理位置。
   */
  location?: boolean;
...
export interface IReGeoCodePois {
  /**
   * poi名称。
   */
  number: string;
...
export interface IReGeoCodeStreetNumber {
  /**
   * 街道 number。
   */
  number: string;
...
export interface IReaddirRequest {
  /**
   * 目录地址
   */
  dirPath: string;
...
export interface IReaddirResponse {
  files: string[];
}

export interface IRegionPickerMergeOptions {
  /**
   * 删除城市信息。
   */
...
export interface IRegionPickerMergeOptionsAdd {
  /**
   * 插入点之上的省份 ID。
   */
  pid: string;
...
export interface IRegionPickerMergeOptionsAddSubList {
  /**
   * 名称
   */
  name: string;
...
export interface IRegionPickerMergeOptionsRemove {
  /**
   * 需要移除的省份 ID。
   */
  id: string;
...
export interface IRegionPickerMergeOptionsUpdate {
  /**
   * 更新对象的 ID。
   */
  id: string;
...
export interface IRegionPickerMergeOptionsUpdateSubList {
  /**
   * 名称
   */
  name: string;
...
export interface IRemoveSavedFileRequest {
  /**
   * 需要删除的文件路径
   */
  filePath: string;
...
export interface IReplyRoomInvitationParams {
  inviter: string;
  roomId: string;
}

export interface IRmdirRequest {
  /**
   * 删除的目录路径
   */
  dirPath: string;
...
export interface ISelectorQueryFieldsOption {
  /**
   * 是否返回节点 id
   */
  id?: boolean;
...
export interface ISelectorQueryToImageCallbackResult {
  /**
   * 节点生成的图片的 base64 编码
   */
  image: string;
...
export interface ISelectorQueryToImageOptions {
  /**
   * 将要生成的图片格式，目前提供 png 和 jpeg 格式。
   */
  imageType: 'png' | 'jpeg';
}

export interface ISetCustomPopMenuMenus {
  /**
   * 自定义菜单名称
   */
  name: string;
...
export interface ISetWifiListWifiList {
  /**
   * Wifi 的 SSID
   */
  SSID: string;
...
export interface IStartSpeechExtraInfo {
  enableDBCallback?: boolean;
}
...
export interface ITextRiskIdentificationResult {
  /**
   * 目标内容文本识别到的类型。
   */
  type: 'keyword' | '0' | '1' | '2';
  ...
  hitKeywords?: string[];
  /**
   * 识别命中得分，最高分为 `100` 分。仅当识别没有命中 `keyword`，但入参中包含了 `广告` 或 `涉政` 或 `涉黄` 时，才会返回该字段。
   */
  score?: string;
}

export interface ITracertHandleUEPDataOptions {
  type: string;
  logLevel?: number;
  actionId: string;
  param4?: object;
...
export interface ITracertOptions {
  /**
   * 埋点 a 位
   */
  spmAPos: string;
...
export interface ITracertPageParams {
  newChinfo?: string;
  chInfo?: string;
  chinfo?: string;
  entityId?: string;
...
export interface ITracertReportUEPDataOptions {
  spm: {
    url: string;
    chInfo: string;
    isSPM: boolean;
...
export interface ITypeBLECharacteristic {
  /**
   * 蓝牙设备特征值的 UUID。
   */
  characteristicId: string;
...
export interface ITypeBLECharacteristic$Properties {
  /**
   * 该特征值是否支持 read 操作。
   */
  read: boolean;
...
export interface ITypeCityInfo {
  /**
   * 城市名。
   */
  name?: string;
...
export interface ITypeReferrerInfo {
  /**
   * 来源小程序。
   */
  appId: string;
...
export interface IUnzipRequest {
  /**
   * 源文件路径
...
export interface IXnnManagerInferenceSuccess {
  tensors: any;
}

export interface IXnnManagerLoadSuccess {
  inputTensors: any;
  outputTensors: any;
}

export interface IXnnManagerTransformFromImageSuccess {
  tensor: any;
}

export interface IXnnManagerTransformFromVideoFrameSuccess {
  tensor: any;
}
 ...
 * 附加信息
 */
export interface IZimIdentityResponseExtInfo {
  /**
   * 人脸源自产品
   */
  zimAction: number;
 ...
 * 含有 verifyId 的返回值
 */
export interface IZimIdentityResponseWithVerifyId {
  /**
 ...
 * 不含有 verifyId 的返回值
 */
export interface IZimIdentityResponseWithoutVerifyId {
  /**
   * 状态码。
   * - `1003` 用户取消刷脸
 ...
 * 跳转到地图页面。
 */
export interface IZmCreditBorrowRequest4OpenZMCreditBorrowMapPage {
  /**
 ...
 * 唤起芝麻信用页面参数。
 */
export interface IZmCreditBorrowRequest4OpenZMCreditBorrowPage {
  /**
...
export interface TypeBluetoothDevice {
  /**
   * 蓝牙设备名称（某些设备可能没有）。
   */
  name: string;
...
export interface TypeDOMMatrix2DInit {
  a?: number;
  b?: number;
  c?: number;
  d?: number;
...
export interface TypeDOMMatrixInit {
  a?: number;
  b?: number;
  c?: number;
  d?: number;
...
export const enum TypeSystemInfo$AppMode {
  /**
   * 标准版
   */
  normal = 'normal',
...
export const enum TypeSystemInfo$Performance {
  /**
   * 高性能
   * @description
...
export interface TypeSystemInfo$SafeArea {
  left: number;
  right: number;
  top: number;
  bottom: number;
...
export interface TypeSystemInfo$Screen {
  /**
   * 屏幕宽度
   */
  width: number;
