...
class PathAliasDefinition {
                ...
                if (lastStatInfo) {
                    ...
                    if (lastPath) {
                        if (lastPath.type === 'file') {
                            ...
                            const currentFileIndexPath = path.resolve(lastPath.absolutePath, `index.${currentDocumentExt}`);
                            const currentFileIndexJsPath = path.resolve(lastPath.absolutePath, `index.js`);
                            if (fs.existsSync(currentFileIndexPath)) {
                                return new vscode_1.Location(vscode_1.Uri.file(currentFileIndexPath), new vscode_1.Position(0, 0));
                            }
                            else if (fs.existsSync(currentFileIndexJsPath)) {
                                return new vscode_1.Location(vscode_1.Uri.file(currentFileIndexJsPath), new vscode_1.Position(0, 0));
                            }
                        }
                    }
                    else {
    ...
    importDefination(document, position) {
        ...
        if (execResult) {
            ...
            if (mostLike) {
                ...
                if (extname === 'js' || extname === 'ts') {
                    ...
                    console.timeEnd('ast');
                    if (retDefination) {
                        return new vscode_1.Location(vscode_1.Uri.file(absolutePathWithExtname), new vscode_1.Position(retDefination.position.line, retDefination.position.character));
                    }
                }
            }
        }
